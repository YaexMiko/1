...
async def get_animes(name, torrent, force=False):
    try:
        ...
        if force or (not (ani_data := await db.getAnime(ani_id)) \
            ...
            # Get channels for this anime from the database
            channels = await db.get_channels_for_anime(name)
            ...
            for channel_id in channels:
                try:
                    ...
                    await rep.report(f"Error sending post to channel {channel_id}: {e}", "error")
            ...
            for qual in Var.QUALS:
                ...
                # Update progress to separate channels
                for channel_id in channels:
                    try:
                        await editMessage(channel_id, f"â€£ <b>Anime Name :</b> <b><i>{name}</i></b>\n\n<i>Ready to Upload...</i>")
                    except Exception as e:
                        await rep.report(f"Error updating progress in channel {channel_id}: {e}", "error")

                # Update main channel as well
                if post_msg:
